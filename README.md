...
Kim et al. (2024) parallelizes independent tool calls within a single agent turn. Agentc targets a different layer: application-level call traces, where model choice, context shape, state retention, semantic reuse, and scheduling can be handled by one runtime policy.

Model routing, prompt compression, semantic caching, workflow optimization, and serving systems are all active areas. Agentc's specific bet is narrower: transparent framework-call interception plus conservative rewrite passes over observed agent traces.
