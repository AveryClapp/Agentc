...
from bench.openrouter_rules_protocol import STAGES, UNAVAILABLE, activation, prompt_constants, protocol_policies, workflow_call
...
ARMS = ("original", "historical_rules", "guarded_rules", "routing_only", "sequential", "joint")
...
def sources():
    paths = ("bench/openrouter_rules_live.py", "bench/openrouter_rules_protocol.py", "bench/openrouter_pilot.py",
        "bench/openrouter_matrix.py", "bench/openrouter_frontier.py", "bench/openrouter_contract.py",
        "bench/openrouter_replay.py", "bench/agents/research_planner.py", "python/agentc/_attention.py")
    return {p: file_hash(ROOT / p) for p in paths}


def runtime_sources():
    """Bind the separately reviewed native build to immutable Git sources."""
    commit = subprocess.check_output(["git", "rev-parse", RUNTIME_COMMIT], cwd=ROOT, text=True).strip()
    paths = subprocess.check_output(["git", "ls-tree", "-r", "--name-only", commit,
                                    "Cargo.toml", "Cargo.lock", "crates"], cwd=ROOT, text=True).splitlines()
    selected = [p for p in paths if p.endswith((".rs", "Cargo.toml", "Cargo.lock"))]
    hashes = {p: hashlib.sha256(subprocess.check_output(["git", "show", commit + ":" + p], cwd=ROOT)).hexdigest()
              for p in selected}
...
def payload_for(call, manifest):
    ...
    if (call["model"] not in manifest["endpoints"] or call.get("tools")
            or type(cap) is not int or not 1 <= cap <= 512 or params.get("temperature") != 0):
        raise PilotError("workflow request exceeds frozen model/parameter coverage")
    return make_request(call["model"], [manifest["endpoints"][call["model"]]["tag"]], call["messages"], max_tokens=cap)
...
def schedule_for(ids, excluded, calibration, development):
    if calibration < 1 or development < 4 or len(ids) != len(set(ids)):
        raise PilotError("need unique tasks, calibration, and at least four development workflows")
    eligible = sorted(set(ids) - set(excluded), key=lambda t: digest(["rules-live-development-v1", t]))
    ...
    for i, task_id in enumerate(eligible[:calibration + development]):
        phase = "calibration" if i < calibration else "warmup" if i < calibration + 3 else "development"
        arms = ["calibration/" + m for m in (SOURCE_MODEL, TARGET_MODEL)] if phase == "calibration" else ARMS
...
def prepare(args, key):
    ...
    if file_hash(args.native) != RUNTIME_SHA256 or os.environ.get("PYTHONHASHSEED") != "0":
    ...
    excluded = set(frontier["excluded_question_ids"]) | {r["task_id"] for r in frontier["schedule"]}
    excluded.update(r["task_id"] for r in previous["schedule"])
    schedule = schedule_for([t["task_id"] for t in tasks], excluded, args.calibration, args.development)
    ...
    for target in catalog["targets"]:
        ...
        target.update(model_version=model + "@" + version + "/" + e["tag"], max_output_tokens=512,
                      context_window_tokens=min(65536, e["context_length"]))
        target["price"] = {"input_per_million_tokens_usd": float(money(e["pricing"]["prompt"]) * 1000000),
            "output_per_million_tokens_usd": float(money(e["pricing"]["completion"]) * 1000000),
    ...
    manifest = {"schema_version": 1, "kind": "live_six_arm_development", "paper_evidence": False,
        "created_at": observed, "source_files": sources(), "native_sha256": file_hash(args.native),
        "runtime": runtime_sources(), "fixture_sha256": file_hash(args.fixture), "context": "extended",
        "frontier_manifest_sha256": digest(frontier), "previous_manifest_sha256": digest(previous),
        "excluded_question_ids": sorted(excluded), "schedule": schedule, "catalog": catalog, "endpoints": selected,
        "stage_cap_usd": "1", "source_model": SOURCE_MODEL, "target_model": TARGET_MODEL,
        "maximum_reconstruction_age_seconds": 7200,
        "policies": {arm: policy_settings(arm) for arm in ARMS}, "prompts": prompt_constants(),
        "shadow_seed": "rules-live-shadow-v1", "scoring": "unchanged_normalized_raw_answer_EM_F1",
        ...
        "risk_margin": .02, "unavailable_rules": UNAVAILABLE,
        "limitations": [
    ...
    attention = load_module("rules_prepare_attention", ROOT / "python/agentc/_attention.py")
    by_id = {t["task_id"]: t for t in tasks}
    for task_id in {r["task_id"] for r in schedule}:
        payload_for(workflow_call(public_task(by_id[task_id]), "filter", {}, attention,
                                 prompts=manifest["prompts"]), manifest)
    write_json(args.output / "manifest.json", manifest, immutable=True)
    return {"manifest_sha256": digest(manifest), "scheduled_workflows": len(schedule),
            "minimum_primary_calls": len(schedule)*3, "stage_cap_usd": "1"}
...
def static_selection(decisions, calls, tasks, manifest):
    ...
    expected_ids = {r["task_id"] for r in manifest["schedule"] if r["phase"] == "calibration"}
    cells = []
    for model in (SOURCE_MODEL, TARGET_MODEL):
        ...
        if (len(rows) != len(expected_ids)*3 or len(answers) != len(expected_ids)
                or {d["task_id"] for d in answers} != expected_ids):
            raise PilotError("static router requires complete independent calibration")
        f1 = sum(score(by_id[d["primary_id"]]["answer"], tasks[d["task_id"]]["expected"])["f1"] for d in answers)/len(answers)
        cost = sum((money(by_id[d["primary_id"]]["nominal_uncached_cost_usd"]) for d in rows), Decimal(0))
        cells.append({"model": model, "mean_final_f1": f1, "nominal_cost_usd": str(cost), "workflows": len(answers)})
    source_f1 = next(c["mean_final_f1"] for c in cells if c["model"] == SOURCE_MODEL)
    eligible = [c for c in cells if c["mean_final_f1"] >= source_f1 - manifest["risk_margin"]]
    selected = min(eligible, key=lambda c: (money(c["nominal_cost_usd"]), c["model"]))["model"]
    return {"model": selected, "cells": cells, "all_candidate_calibration_costs_charged": True,
            "safety_guarantee": False, "calibration_only": True}
...
def summarize(decisions, calls, manifest, tasks):
    ...
    for arm in ARMS:
        ...
        answers = [d for d in selected if d["workflow_stage"] == "answer" and d["phase"] == "development"]
        charged = [r for r in calls if r["arm"] == arm] + (calibration if arm == "sequential" else [])
        reports.append({"arm": arm, "completed_decisions": len(selected), "development_workflows": len(answers),
            "development_mean_f1": sum(score(by_id[d["primary_id"]]["answer"], tasks[d["task_id"]]["expected"])["f1"] for d in answers)/len(answers) if answers else None,
            "setup_inclusive_billed_usd": str(sum((money(r["cost_usd"]) for r in charged), Decimal(0))),
            "setup_inclusive_nominal_usd": str(sum((money(r["nominal_uncached_cost_usd"]) for r in charged), Decimal(0))),
            "calls_by_scope": dict(Counter(r["scope"] for r in charged)),
            "primary_model_counts": dict(Counter(by_id[d["primary_id"]]["model"] for d in selected)),
            "selected_rule_counts": dict(Counter(r for d in selected for r in d["activation"]["selected_rules"])),
            "candidate_rule_counts": dict(Counter(r for d in selected for r in d["semantic_plan"]["candidate_rules"])),
            "truncated_calls": sum(r["finish_reason"] == "length" for r in charged),
            "cached_input_tokens": sum(r["cached_input_tokens"] or 0 for r in charged),
            "cache_accounting_missing_calls": sum(r["cached_input_tokens"] is None for r in charged)})
    return {"paper_evidence": False, "evaluation_kind": manifest["kind"], "manifest_sha256": digest(manifest),
        "decisions_sha256": digest(decisions), "calls_sha256": digest(calls), "completed_calls": len(calls),
        "completed_decisions": len(decisions), "scheduled_decisions": len(manifest["schedule"])*3,
...
class Acquisition:
    ...
    def validate_journals(self):
        ...
        steps = [{**workflow, "workflow_stage": stage}
                 for workflow in self.manifest["schedule"] for stage in STAGES]
        ...
        for i, intent in enumerate(self.intents):
            ...
            signature = semantic_plan(intent["original_call"], intent["native_plan"], self.manifest)
            if signature != intent["semantic_plan"]:
                raise PilotError("workflow intent signature changed")
            scopes = [("calibration" if item["phase"] == "calibration" else "primary", signature["primary"])]
            if signature["candidate"] is not None:
                scopes.append(("exploration", signature["candidate"]))
            elif signature["kind"] != "pass_through" and shadow_sample(self.manifest["shadow_seed"],
                    item["arm"] + "/" + item["workflow_stage"], item["task_id"],
                    float(self.manifest["policies"][item["arm"]]["AGENTC_OPTIMIZE_SHADOW"])):
                scopes.append(("shadow", payload_for(intent["original_call"], self.manifest)))
            ids = []
            for scope, payload in scopes:
                call_id = self.stage + "-" + digest([item, scope])[:24]
                ids.append(call_id)
                expected_calls.append({**item, "scope": scope, "id": call_id, "stage": self.stage,
            ...
            if i < len(self.decisions):
                ...
                if (any(decision.get(k) != v for k, v in item.items()) or decision["semantic_plan"] != signature
                        or decision["primary_id"] != ids[0] or decision["incurred_ids"] != ids):
        ...
        if self.decisions and completed_lengths[len(self.decisions)-1] > len(self.calls):
    ...
    def intent(self, item, call, plan):
        record = {**item, "original_call": call, "semantic_plan": semantic_plan(call, plan, self.manifest),
                  "native_plan": plan}
        index = self.decision_index
        if index < len(self.intents):
            if {k: v for k, v in self.intents[index].items() if k != "native_plan"} != {k: v for k, v in record.items() if k != "native_plan"}:
                raise PilotError("native decision changed during resume; no further dispatch")
        else:
    ...
    def dispatch(self, item, scope, call, signature):
        ...
        metadata = {**item, "scope": scope, "manifest_sha256": digest(self.manifest),
            "decision_sha256": digest(signature), "dispatch_contract": {
        ...
        row = {**item, "scope": scope, **{k: v for k, v in result.items() if k not in {"key_id", "at", "metadata"}},
            "request_sha256": digest(payload), "decision_sha256": digest(signature), "cached_input_tokens": cached,
            "nominal_uncached_cost_usd": str(money(e["pricing"]["prompt"])*result["usage"]["prompt_tokens"]
                + money(e["pricing"]["completion"])*result["usage"]["completion_tokens"])}
        if self.call_index < len(self.calls):
            ...
            print(json.dumps({"completed_calls": len(self.calls), **item, "scope": scope,
                "model": row["model"], "cost_usd": row["cost_usd"]}), flush=True)
        self.call_index += 1
        return row
    ...
    def record(self, record):
        if self.decision_index < len(self.decisions):
            ...
            if {k: v for k, v in saved.items() if k != "native_plan"} != {k: v for k, v in record.items() if k != "native_plan"}:
...
def run(args, key):
    ...
    if (manifest["source_files"] != sources() or manifest["native_sha256"] != file_hash(args.native)
            or manifest["runtime"] != runtime_sources() or manifest["fixture_sha256"] != file_hash(args.fixture)
            or os.environ.get("PYTHONHASHSEED") != "0"):
        raise PilotError("frozen workflow sources, native, fixture, or hash seed changed")
    ...
    if not 0 <= age <= manifest["maximum_reconstruction_age_seconds"]:
        raise PilotError("development reconstruction window expired; no further dispatch")
    ...
    attention = load_module("live_rules_attention", ROOT / "python/agentc/_attention.py")
    ...
    try:
        with ExitStack() as stack:
            stores = {arm: stack.enter_context(tempfile.TemporaryDirectory(prefix="agentc-live-" + arm + "-")) for arm in ARMS}
            for workflow in manifest["schedule"]:
                arm = workflow["arm"]
                is_calibration = workflow["phase"] == "calibration"
                if not is_calibration and selector is None:
                    selector = static_selection(acquisition.decisions, acquisition.calls, tasks, manifest)
                    write_json(args.output / "static-selection.json", selector, immutable=True)
                model = arm.removeprefix("calibration/") if is_calibration else selector["model"] if arm == "sequential" else SOURCE_MODEL
                clear_env()
                os.environ.update(manifest["policies"]["original" if is_calibration else arm])
                if not is_calibration:
                    native.optimize_configure(stores[arm], catalog_json=json.dumps(manifest["catalog"]))
                ...
                try:
                    for stage in STAGES:
                        ...
                        call = workflow_call(public[workflow["task_id"]], stage, history, attention, model=model,
                            run_identity=acquisition.stage + "/" + arm, prompts=manifest["prompts"])
                        encoded = json.dumps({"kind": "pass_through"}) if is_calibration else native.optimize_plan(json.dumps(call))
                        plan = json.loads(encoded)
                        signature = acquisition.intent(item, call, plan)
                        primary = acquisition.dispatch(item, "calibration" if is_calibration else "primary", plan.get("call", call), signature)
                        incurred = [primary["id"]]
                        divergence = None
                        if not is_calibration:
                            token = native.optimize_observe(encoded, json.dumps(outcome(primary, call["call_site_id"])))
                            if arm != "original" and not token:
                                raise PilotError("unattributable live primary observation")
                            exploration = plan.get("agentc_exploration_context")
                            if exploration:
                                candidate = acquisition.dispatch(item, "exploration", exploration["candidate_plan"]["call"], signature)
                                ...
                                if not native.optimize_complete_exploration(exploration["lease_token"],
                                    ...
                                    raise PilotError("live exploration completion rejected")
                            elif plan["kind"] != "pass_through" and shadow_sample(manifest["shadow_seed"], arm + "/" + stage,
                                    workflow["task_id"], float(manifest["policies"][arm]["AGENTC_OPTIMIZE_SHADOW"])):
                                reference = acquisition.dispatch(item, "shadow", call, signature)
                        ...
                        acquisition.record({**item, "native_plan": plan, "semantic_plan": signature,
                            "primary_id": primary["id"], "incurred_ids": incurred, "activation": active,
            ...
            if (acquisition.call_index != len(acquisition.calls)
                    or acquisition.decision_index != len(acquisition.decisions)
                    or acquisition.decision_index != len(acquisition.intents)):
                raise PilotError("workflow reconstruction left an unconsumed journal suffix")
...
def main():
    ...
    parser.add_argument("--frontier", type=Path, default=ROOT / "bench/repro/openrouter-frontier-2026-09-04/manifest.json")
