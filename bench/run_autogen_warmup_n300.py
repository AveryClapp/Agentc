"""Revision Task 3b: autogen_bridge at n=300 under warmup-corrected harness.

Powers up the published n=200 accuracy-neutrality test (all-on -3.0pp, p=0.07)
to n=300 for a cleaner read on whether the accuracy delta is real or noise.
Reuses the existing 300-task long_context_qa_n300.json fixture directly. Also
writes a per-task CSV so paired_analysis can attach bootstrap CIs.

11-config ablation (all-on, 5x-off, 5x-only). autogen_bridge runs 2
LLM calls per task: autogen.reason (long doc + state_write tool result)
and autogen.answer (same long doc; tool_result state-tagged but not
re-read -> SD candidate). Tracks CC and SD fire counts separately.

Cost estimate: ~$5-9 (Together Llama-3.3-70B). Ceiling: $12 warn / $18 abort.

Outputs:
  bench/paper_results/autogen_bridge_warmup_n300.csv
  bench/paper_results/autogen_bridge_warmup_n300_warmup_stats.csv
  bench/paper_results/autogen_bridge-n300-warmup.per_task.csv
"""
...
W_TASKS = int(os.environ.get("AB_W", "30"))
N_TASKS = int(os.environ.get("AB_N", "300"))
COST_CEILING_USD = 12.0
ABORT_CEILING_USD = 18.0
PAPER_RESULTS = _REPO / "bench" / "paper_results"
STORAGE_ROOT = Path("/tmp/agentc-autogen-warmup-n300")
FIXTURE_N300 = _REPO / "bench" / "fixtures" / "long_context_qa_n300.json"
FIXTURE_SRC = _REPO / "bench" / "fixtures" / "hotpot_distractor.json"
OUT_PATH = PAPER_RESULTS / "autogen_bridge_warmup_n300.csv"
WARMUP_STATS_PATH = PAPER_RESULTS / "autogen_bridge_warmup_n300_warmup_stats.csv"
PER_TASK_PATH = PAPER_RESULTS / "autogen_bridge-n300-warmup.per_task.csv"
...
_cfg_override = os.environ.get("AB_CONFIGS", "").strip()
if _cfg_override:
...
def _ensure_fixture() -> None:
    ...
    if not FIXTURE_SRC.is_file():
        raise RuntimeError(
            f"Source fixture missing: {FIXTURE_SRC}\n"
            "Run `python -m bench.build_hotpot_fixture` first, then run_lcqa_warmup_n300.py."
        )
    print("  fixture: building long_context_qa_n300.json from hotpot_distractor.json ...")
    ...
    for task in src[:300]:
        ...
        added: list = []
        attempts = 0
        while len(added) < 10 and attempts < 200:
    ...
    print(f"  fixture: built {len(out)} tasks → {FIXTURE_N300}")
...
def _run_phase(
    ...
    if not together_key:
        raise RuntimeError("TOGETHER_API_KEY not set — add it to .env")
    env["BENCH_OPENAI_BASE_URL"] = _TOGETHER_BASE_URL
    env["BENCH_BASELINE_MODEL"] = _TOGETHER_MODEL
    if extra_env:
...
def _query_rule_fires(storage_dir: Path, rule: str) -> int:
...
def main() -> int:
    ...
    for config in _CONFIGS:
        ...
        cc_fires = _query_rule_fires(opt_dir, "ContextCompress")
        sd_fires = _query_rule_fires(opt_dir, "StateDrop")

        print(f"  {n_pass}/{N_TASKS}  BF={n_BF} FB={n_FB} p={p_val:.4f}  "
              f"cost_saved={cost_saved:+.4f} mUSD  tok_saved={tok_saved_pct:+.2f}%  "
              f"CC={cc_fires}  SD={sd_fires}")
        _append_row({
