...
def _run_side(
    ...
    # Count only tasks that actually PASSED. The missing `if p` filter counted
    # every task as a pass, forcing baseline/optimized acc_pct to 100.0 by
    # construction (bd-r0ln). The McNemar cells below use the real booleans, so
    # the deltas the paper cites were unaffected — but these acc_pct columns
    # were fabricated. NOTE: the committed generalization_evals.csv still holds
    # the old 100% values; it must be regenerated by a re-run to pick this up.
    n_passed = sum(1 for _, p in per_task if p)
