"""Generic guard-evaluation runner.

Runs baseline + chosen ablation configs for an arbitrary agent, honoring the
shadow-divergence accuracy guard (AGENTC_OPTIMIZE_SHADOW /
AGENTC_SHADOW_DIVERGENCE_BUDGET are inherited from the environment). Writes a
summary CSV + a paired_analysis-compatible per-task CSV, and records CC/SD fire
counts (including composed plans).

Env:
  GE_AGENT     required, e.g. bench.agents.long_context_qa
  GE_FIXTURE   optional, path for BENCH_FIXTURE_OVERRIDE
  GE_CONFIGS   comma list, default "ContextCompress-only"
  GE_N, GE_W   measurement / warmup task counts (default 150 / 20)
  GE_TAG       output filename tag (default "guard_eval")
...
W_TASKS = int(os.environ.get("GE_W", "20"))
N_TASKS = int(os.environ.get("GE_N", "150"))
TAG = os.environ.get("GE_TAG", "guard_eval")
COST_CEILING_USD = 6.0
ABORT_CEILING_USD = 10.0
...
_CONFIGS = [c.strip() for c in os.environ.get("GE_CONFIGS", "ContextCompress-only").split(",") if c.strip()]
...
def _run_phase(storage_dir: Path, optimize: bool, n_tasks: int):
    ...
    if FIXTURE:
        env["BENCH_FIXTURE_OVERRIDE"] = FIXTURE
...
def _fires(storage_dir: Path) -> tuple[int, int, int]:
    ...
    conn = sqlite3.connect(str(db))
    try:
        cc = conn.execute("SELECT COUNT(*) FROM plan_audit WHERE rule='ContextCompress' AND plan_kind IN ('rewritten','composed')").fetchone()
        sd = conn.execute("SELECT COUNT(*) FROM plan_audit WHERE rule='StateDrop' AND plan_kind IN ('rewritten','composed')").fetchone()
        total = conn.execute("SELECT COUNT(*) FROM plan_audit").fetchone()
        return (int(cc[0]) if cc else 0, int(sd[0]) if sd else 0, int(total[0]) if total else 0)
    finally:
        conn.close()
