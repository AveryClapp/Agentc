"""Section 6.4 warmup-corrected re-verification: hotpot_qa, N=300, W=30, gpt-4o-mini.

11-config ablation (all-on, 5x-off, 5x-only) under warmup-corrected harness.
Per-config isolated optimizer state: separate traces.db, optimizer_audit.db,
and cost_model.db per config. W=30 warmup tasks populate cost_model.db; then
traces.db and optimizer_audit.db are deleted before N=300 measurement tasks run.

Fixture: bench/fixtures/hotpot_distractor.json (500 tasks; BENCH_MAX_TASKS=300
takes the first 300). Prompt size median ~6,940 bytes; only ~82/300 exceed the
8,192-byte CC activation threshold, so the expected CC fire rate is low (~1/300).

Cost estimate: under $2 (prompts shorter than long_context_qa). Ceiling: $8
warn / $12 abort.

Outputs:
  bench/paper_results/hotpot_real-contextcompress-n300-warmup.csv
  bench/paper_results/hotpot_real-contextcompress-n300-warmup_stats.csv
"""
...
AGENT = "bench.agents.hotpot_qa"
W_TASKS = 30
N_TASKS = 300
COST_CEILING_USD = 8.0
ABORT_CEILING_USD = 12.0
PAPER_RESULTS = _REPO / "bench" / "paper_results"
STORAGE_ROOT = Path("/tmp/agentc-hotpot-warmup-n300")
OUT_PATH = PAPER_RESULTS / "hotpot_real-contextcompress-n300-warmup.csv"
WARMUP_STATS_PATH = PAPER_RESULTS / "hotpot_real-contextcompress-n300-warmup_stats.csv"
...
def main() -> int:
    ...
    for config in _CONFIGS:
        ...
        # Verify CC-off configs have 0 fires
        if "ContextCompress-off" in config or config.endswith("-only") and "ContextCompress" not in config:
            if cc_fires > 0:
                print(f"  WARNING: CC fired {cc_fires} times in {config} where it should not")
    ...
    # Final CC fire verification across all completed configs
    print("\n=== CC fire verification ===")
    all_on_fires = _query_cc_fires(STORAGE_ROOT / "all-on" / "optimized")
    cc_only_fires = _query_cc_fires(STORAGE_ROOT / "ContextCompress-only" / "optimized")
    cc_off_fires = _query_cc_fires(STORAGE_ROOT / "ContextCompress-off" / "optimized")
    print(f"all-on CC fires: {all_on_fires}/300")
    print(f"ContextCompress-only CC fires: {cc_only_fires}/300")
    print(f"ContextCompress-off CC fires: {cc_off_fires}/300 (must be 0)")
    if cc_off_fires > 0:
        print("ERROR: CC fired on CC-off config -- precondition violation")
        return 1
    if all_on_fires > 50:
        print(f"WARNING: CC fires ({all_on_fires}/300) substantially exceed expected ~1/300")
        print("STOP: contamination may be suppressing fires. Surface before updating paper.")
        return 1
