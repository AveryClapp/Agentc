"""Section 6.1 re-verification: long_context_qa, N=100, W=30 warmup, gpt-4o-mini.
...
Cost ceiling: $20 hard stop ($30 surface-and-abort).

Outputs:
  bench/paper_results/long_context_qa_warmup_n100.csv
  bench/paper_results/long_context_qa_warmup_n100_warmup_stats.csv
"""
...
STORAGE_ROOT = Path("/tmp/agentc-lcqa-warmup")
OUT_PATH = PAPER_RESULTS / "long_context_qa_warmup_n100.csv"
WARMUP_STATS_PATH = PAPER_RESULTS / "long_context_qa_warmup_n100_warmup_stats.csv"
...
def _log_warmup_state(storage_dir: Path) -> list[tuple[str, int]]:
    """Return [(call_site_id, n_observations)] from cost_model.db."""
...
def main() -> int:
    ...
    se_b = 100.0 * math.sqrt(b_acc / 100.0 * (1 - b_acc / 100.0) / N_TASKS) if N_TASKS else 0.0
