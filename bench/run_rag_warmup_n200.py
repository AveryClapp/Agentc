"""rag_summarizer at n=200 under warmup-corrected harness.

11-config ablation (all-on, 5x-off, 5x-only). rag_summarizer runs 3
LLM calls per task: 2x rag.chunk_summary (fan-out) + 1x rag.combine.
CC may fire on chunk_summary calls if chunk context exceeds 8 KB gate.

bench/fixtures/rag_summarizer.json has 200 tasks; no fixture override
needed.

Cost estimate: ~$2-4. Ceiling: $15 hard stop ($25 surface-and-abort).

Outputs:
  bench/paper_results/rag_summarizer_warmup_n200.csv
  bench/paper_results/rag_summarizer_warmup_n200_warmup_stats.csv
"""
...
AGENT = "bench.agents.rag_summarizer"
...
STORAGE_ROOT = Path("/tmp/agentc-rag-warmup-n200")
OUT_PATH = PAPER_RESULTS / "rag_summarizer_warmup_n200.csv"
WARMUP_STATS_PATH = PAPER_RESULTS / "rag_summarizer_warmup_n200_warmup_stats.csv"
