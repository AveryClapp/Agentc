"""Section 6.3 re-verification: iterative_refiner, N=50, W=30 warmup, gpt-4o-mini.
...
AGENTC_COMPOSE=1 on all optimized configs. temperature=0 is hardcoded in agent.

Cost ceiling: $15 hard stop ($25 surface-and-abort).

Outputs:
  bench/paper_results/iterative_refiner_warmup_n50.csv
  bench/paper_results/iterative_refiner_warmup_n50_warmup_stats.csv
"""
...
AGENT = "bench.agents.iterative_refiner"
W_TASKS = 30
N_TASKS = 50
COST_CEILING_USD = 15.0
ABORT_CEILING_USD = 25.0
PAPER_RESULTS = _REPO / "bench" / "paper_results"
STORAGE_ROOT = Path("/tmp/agentc-refiner-warmup")
OUT_PATH = PAPER_RESULTS / "iterative_refiner_warmup_n50.csv"
WARMUP_STATS_PATH = PAPER_RESULTS / "iterative_refiner_warmup_n50_warmup_stats.csv"
...
_CSV_COLUMNS = [
    ...
    "sd_fire_count", "sd_total_calls", "sd_window_reads_present", "sd_state_tagged_present",
]
_WARMUP_COLS = [
...
def _query_sd_stats(storage_dir: Path) -> tuple[int, int]:
    """Return (sd_fire_count, sd_total_refiner_calls) from optimizer_audit.db."""
...
def main() -> int:
    ...
    for config in _CONFIGS:
        ...
        sd_fires, sd_total = _query_sd_stats(opt_dir)
        sd_window = 1 if sd_fires > 0 else 0
        sd_tagged = 1 if sd_fires > 0 else 0

        print(f"  {n_pass}/{N_TASKS}  BF={n_BF} FB={n_FB} p={p_val:.4f}  "
              f"cost_saved={cost_saved:+.4f} mUSD  tok_saved={tok_saved_pct:+.2f}%  "
              f"SD_fires={sd_fires}/{sd_total}")
        ...
        _append_row({
            ...
            "sd_total_calls": sd_total,
            "sd_window_reads_present": sd_window,
            "sd_state_tagged_present": sd_tagged,
        })
