...
ablation coverage in Section~\ref{sec:eval-hotpot}. All results were produced for
under \$25 of API spend across OpenAI, Anthropic, and Together AI.
...
\textbf{[FLAG]} No dedicated warmup runner exists for \texttt{hotpot\_qa}; the
original command used shared optimizer state. A correct re-run would require adapting
\texttt{run\_\allowbreak lcqa\_\allowbreak warmup\_\allowbreak n300.py} with
\texttt{AGENT=bench.agents.hotpot\_qa} and a real HotpotQA fixture, but no such
script exists in \texttt{bench/}. \\
