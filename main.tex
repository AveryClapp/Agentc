...
%% ============================================================
%% DROPIN-INDEX (MLSys revision) -- delete before submission.
%% Pending values to drop in once experiments finish; search '%% DROPIN':
%%  [3a] DONE: research_planner per-agent result applied (CC-only +4pp/37.7%; SD -7pp harmful)
%%       -> tab:summary row + Limitations + guard section (sec:eval-guard).
%%  [GUARD] DONE: shadow-divergence accuracy guard built + validated (Path A) -> sec:eval-guard,
%%       contribution bullet 4, limitation. analyst SD -42.7->-1.3 (guard on); CC 37.7->11 (conservative).
%%  [3b] DONE: autogen n=300 clean shared-baseline (CC-only 23.5%/-1.3pp/p=0.29 ns; composed 14.0%/-1.0pp/p=0.55 ns) -- both neutral, no safety frontier
%%       -> tab:summary (2 rows), Sec eval-realagent rewrite, abstract/intro/conclusion qualified.
%%  [T4] DONE: throughput-under-concurrency figure wired in (fig:throughput), real n=50 data.
%%  [T2] naive-baseline attribution (random/recency/bm25 vs IDF) -- NOT YET RUN
%% ============================================================
...
label-free accuracy guard, at a bounded 2\% shadow-sampling cost, converts a $-$42.7\,pp
StateDrop catastrophe to $-$1.3\,pp (97\% of the damage prevented) while keeping beneficial
compression enabled; the containment metric that makes it selective reproduces across four
...
the remaining limit is lexically-invisible semantic disagreement
...
The mechanism is rule-agnostic and requires no ground-truth labels. It samples at a
configurable rate (2\% in production), so the guard's safety costs roughly 2\% extra
inference in the steady state, a bounded and tunable cost/safety knob; a rule that
auto-disables thereafter incurs no further shadow cost. The guard's own bookkeeping is
negligible against the call it guards: computing the divergence metric and folding it into
the Rust budget takes 18\,$\mu$s per sampled call (17.8\,$\mu$s metric, 0.7\,$\mu$s fold;
\path{bench/guard_overhead_bench.py}), five orders of magnitude below an LLM call. The
shadow \emph{inference} call itself, however, is a real (billed) API call issued
synchronously on the sampled requests, so it adds that call's full latency and cost to
those $\approx$2\% of requests; unsampled calls are unaffected, and the rate is tunable.
...
compression. All guard-on accuracy deltas are non-degrading.}
\label{tab:guard}
\small
\begin{tabular}{lllr}
\toprule
...
$\tau$=0.10. $n$=100 ($n$=200 for gpt-4o-mini).}
\label{tab:xmodel}
\small
\begin{tabular}{llllr}
\toprule
