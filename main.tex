...
compression enabled; the containment metric that makes it selective reproduces across model
families. On
...
\textbf{The selectivity is model-agnostic.} The lexical-versus-normalized behavior is not a
quirk of one model. Re-running both agents on Llama-3.3-70B (a different family and provider,
via an OpenAI-compatible endpoint, $n$=100) reproduces it: unguarded ContextCompress on
\texttt{research\_planner} fires 93 of 100 calls at $+$3.0\,pp (ns) and 29.9\% input-token
savings, the lexical guard disables it after 5 fires (savings collapse to 1.6\%), while the
normalized metric keeps it at the full unguarded rate (93 fires, 29.9\% savings preserved,
$+$2.0\,pp ns). Both metrics still disable catastrophic StateDrop ($-$26.0\,pp unguarded
$\rightarrow$ $-$12 to $-$17\,pp), though the recovery is less complete than on the GPT model.
Across both model families it is the divergence \textit{metric}, not the model, that decides
whether a benign output-changing rule survives.
