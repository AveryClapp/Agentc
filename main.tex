...
savings from the validated 11.4\% to a spurious 35.3\%. Fix: per-configuration warmup
...
four real agents, one unseen cold agent, and three model providers.
...
ModelDowngrade savings are structural: price per token, not token count (11.2\,mUSD
...
total. Model: \texttt{gpt-4o-mini}. Ten of eleven ablation configurations completed;
the StateDrop-only configuration at temperature 1 is estimated using a temperature-0
controlled trial.
...
\textbf{Accuracy.} StateDrop in isolation is accuracy-neutral: SD-only achieves
0.0\,pp accuracy delta (McNemar $p$=1.0, SE $\pm$2.0\,pp, $n$=50, baseline pass rate
98\%). Non-zero accuracy deltas observed in multi-rule configurations (all-on:
$-$4.0\,pp; CacheHit-off: $-$6.0\,pp) are attributable to other rules active in
those configurations, not to StateDrop; all are not statistically significant
(McNemar $p \geq 0.25$ throughout). The StateDrop-off isolation confirms the
mechanism: SD-off shows $-$4.0\,pp, identical to configurations where SD is absent,
while SD-only shows 0.0\,pp.
...
\texttt{gpt-4o-mini}). Prompts are not length-expanded; median size is 8,269 bytes.
Only a subset exceeds the 8,192-byte activation threshold. Seven of eleven
configurations complete due to rate limits.

\textbf{Results.} ContextCompress fires on 1 of 300 calls, producing negligible
aggregate savings. For prompts near the activation threshold, the rule correctly
avoids execution when projected savings do not exceed overhead. On fully activated
workloads (Section~\ref{sec:eval-cc}), the rule produces substantial reductions,
indicating that performance depends primarily on activation frequency. The correct
abstention behavior on near-boundary natural prompts is also documented in
Section~\ref{sec:eval-llmlingua}.

\begin{table}[t]
\caption{ContextCompress on HotpotQA-distractor ($n$=300). Rule fires 1 of 300 calls;
median prompt 8,269 bytes sits at the activation boundary.}
...
\textbf{Rate limits and experiment scale.} OpenAI Tier-1 rate limits constrained
ablation coverage in Section~\ref{sec:eval-hotpot} and prevented completion of one
StateDrop configuration at temperature 1. All results were produced for under \$25 of
...
\texttt{BENCH\_\allowbreak MAX\_\allowbreak TASKS=300 python -m bench.optimizer\_\allowbreak ablation bench.agents.long\_\allowbreak context\_\allowbreak qa} \\

\addlinespace
\ref{tab:md-matrix} &
\texttt{gaia\_\allowbreak router-\allowbreak modeldowngrade-\allowbreak n127-\allowbreak warmup.csv} &
\texttt{BENCH\_\allowbreak BASELINE\_\allowbreak MODEL=gpt-4o BENCH\_\allowbreak MAX\_\allowbreak TASKS=127 python -m bench.optimizer\_\allowbreak ablation bench.agents.gaia\_\allowbreak router} \\

\addlinespace
\ref{tab:sd-matrix} &
\texttt{iterative\_\allowbreak refiner-\allowbreak statedrop-\allowbreak n50-\allowbreak warmup.csv} &
\texttt{BENCH\_\allowbreak MAX\_\allowbreak TASKS=50 python -m bench.optimizer\_\allowbreak ablation bench.agents.iterative\_\allowbreak refiner} \\

\addlinespace
\ref{tab:hotpot-matrix} &
\texttt{hotpot\_\allowbreak real-\allowbreak contextcompress-\allowbreak n300-\allowbreak partial7of11.csv} &
\texttt{BENCH\_\allowbreak MAX\_\allowbreak TASKS=300 python -m bench.optimizer\_\allowbreak ablation bench.agents.hotpot\_\allowbreak qa} \\
...
\texttt{BENCH\_\allowbreak MAX\_\allowbreak TASKS=50 python -m bench.optimizer\_\allowbreak ablation bench.agents.composition\_\allowbreak qa} \\

\addlinespace
\ref{tab:ccsd-composition} &
\texttt{multirule\_\allowbreak qa-\allowbreak ccsd-\allowbreak n30-\allowbreak warmup.csv} &
\texttt{BENCH\_\allowbreak MAX\_\allowbreak TASKS=30 python -m bench.optimizer\_\allowbreak ablation bench.agents.multirule\_\allowbreak qa} \\
