...
($-$42.7\,pp on an unseen agent) into a non-significant delta ($-$1.3\,pp) with no
ground-truth labels, while \textit{preserving} benign answer-preserving compression
(ContextCompress on long-context QA stays enabled at 37.6\% savings under the guard).
We also characterize its ceiling: a lexical divergence signal conservatively suppresses
output-\textit{changing}-but-benign rules, the fundamental limit of label-free runtime
gating (Section~\ref{sec:eval-guard}).
\end{itemize}
...
\textbf{Its limit is lexical, not semantic.} Divergence is measured against the
unrewritten output, so the guard detects \textit{change}, not \textit{degradation}, using
a lexical (1\,$-$\,Jaccard) signal. When a rule changes the answer text without making it
worse, the guard cannot tell. On \texttt{research\_planner}, where ContextCompress is
beneficial but \textit{output-changing} ($+$4\,pp, 37.7\% savings), the guard disables it
after 37 fires, cutting savings to 11.0\% ($-$1.0\,pp). This is the fundamental ceiling of
label-free runtime gating: it bounds degradation and preserves stable savings, but a
lexical divergence signal conservatively suppresses output-altering-but-benign rules; a
semantic divergence meter would tighten this. The divergence threshold tunes the
safety/savings tradeoff.

\begin{table}[t]
\caption{The shadow-divergence accuracy guard is divergence-selective: it keeps a rule
whose output agrees with the unrewritten reference (ContextCompress on
\texttt{long\_context\_qa}) and disables one whose output drifts, whether catastrophically
(StateDrop on \texttt{analyst\_qa}) or benignly but lexically (ContextCompress on
\texttt{research\_planner}). All accuracy deltas with the guard on are not significant.}
\label{tab:guard}
\small
\begin{tabular}{llrr}
\toprule
...
\midrule
long\_context\_qa / CC       & off          & $+$0.3\,pp (ns)      & 34.0\% \\
long\_context\_qa / CC       & \textbf{on}  & \textbf{$+$0.7\,pp (ns)} & \textbf{37.6\% (kept)} \\
\midrule
analyst\_qa / StateDrop      & off          & $-$42.7\,pp          & --- \\
analyst\_qa / StateDrop      & \textbf{on}  & \textbf{$-$1.3\,pp (ns)} & disabled \\
\midrule
research\_planner / CC       & off          & $+$4.0\,pp (ns)      & 37.7\% \\
research\_planner / CC       & \textbf{on}  & $-$1.0\,pp (ns)      & 11.0\% (disabled) \\
\bottomrule
\end{tabular}
\end{table}
...
\textbf{The accuracy guard uses a lexical divergence signal.} The shadow-divergence guard
keys on change from the unrewritten output, measured lexically (1\,$-$\,Jaccard). It
preserves stable answer-preserving rules (ContextCompress on \texttt{long\_context\_qa}
stays enabled, 37.6\% savings) and bounds catastrophic degradation (StateDrop), but it
cannot tell a benign answer-text change from a harmful one, so it conservatively disables
output-\textit{changing}-but-benign rules (ContextCompress on \texttt{research\_planner},
37.7\%\,$\rightarrow$\,11.0\%). A semantic divergence meter would tighten this; the
threshold tunes the safety/savings tradeoff. Label-free runtime gating cannot preserve
every benign optimization.
