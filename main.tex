...
%%  [3b] DONE: autogen n=300 frontier applied (CC-only 38.8%/-3.3pp/p=0.02; composed 13.1%/-0.7pp/p=0.77)
...
workloads; at its composed, accuracy-preserving operating point real-agent accuracy
deltas are not statistically significant across three model providers (OpenAI,
Anthropic, open-source Llama-3.3-70B), and composition is what makes aggressive
single-rule compression safe. On
...
accuracy change at its composed operating point; where an aggressive single-rule
configuration would trade accuracy for higher savings, the CompositionPlanner recovers
neutrality (Section~\ref{sec:eval-realagent}).
...
Real-agent accuracy deltas are not statistically significant at the composed operating
point; the aggressive CC-only autogen point trades accuracy for higher savings
(Section~\ref{sec:eval-realagent}).
...
\midrule
autogen (CC-only)   & real agent (Llama\,3.3-70B)  & CC            & ---$^\S$ & 38.8\% & $-$3.3\,pp ($p$=0.02) & 0.02 \\
autogen (composed)  & real agent (Llama\,3.3-70B)  & CC~+~StateDrop & ---$^\S$ & 13.1\% & $-$0.7\,pp (ns, $p$=0.77) & 0.77 \\
...
\textbf{A savings/safety frontier.} autogen exposes a tradeoff between an aggressive and
a safe operating point (Table~\ref{tab:summary}). Run with ContextCompress alone, CC
fires on 597 of 600 calls (99.5\%) and reaches 38.8\% input-token savings, but at a
statistically significant accuracy cost: 73.3\% versus 76.7\% baseline, $-$3.3\,pp,
McNemar exact $p$=0.021 (BF=13, FB=3, 95\% bootstrap CI $[-6.0,-1.0]$\,pp). On this
agent, dropping whole low-attention messages is too aggressive: some removed messages
carry answer-relevant content. The CompositionPlanner recovers safety. In the all-on
configuration, StateDrop fires first (231 of 600 calls), removing stale state-tagged
messages and shrinking many prompts below ContextCompress's activation gate, so CC
fires on only 172 calls. The composed result is accuracy-neutral: 75.7\% versus
76.3\% baseline, $-$0.7\,pp, McNemar $p$=0.77 (BF=7, FB=5, 95\% CI $[-3.0,+1.7]$\,pp),
at 13.1\% input-token savings. Composition thus converts an aggressive,
accuracy-degrading compression into a safe one, trading roughly two thirds of the peak
savings for accuracy neutrality. The composed configuration is the deployable operating
point; the aggressive point is available when accuracy headroom permits.
...
Llama-3.3-70B under the warmup-corrected harness. The composed configuration is
accuracy-neutral ($-$0.7\,pp, $p$=0.77), while ContextCompress alone significantly
degrades accuracy ($-$3.3\,pp, $p$=0.02) at higher savings; we report both as a
savings/accuracy frontier rather than a single number, and treat the composed
configuration as the deployable point (Section~\ref{sec:eval-realagent}).
...
change at its composed operating point; on a real agent where single-rule compression
alone would degrade accuracy, the CompositionPlanner exposes a savings/accuracy frontier
and restores neutrality (Section~\ref{sec:eval-realagent}). Input-token savings are within 1\,pp across
