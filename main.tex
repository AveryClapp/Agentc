...
workloads; real-agent accuracy deltas are not statistically significant across three
model providers (OpenAI, Anthropic, open-source Llama-3.3-70B), and an automatic,
label-free accuracy guard, evaluated with full shadow sampling to characterize its
disable decisions, converts a $-$42.7\,pp StateDrop catastrophe to $-$1.3\,pp (97\%
of the damage prevented) while keeping beneficial compression enabled; the containment
metric that makes it selective reproduces across four
model families (OpenAI, Meta, Anthropic, Alibaba) at a per-model budget. On
...
accuracy change at either its aggressive single-rule or its composed operating point;
the rare rule that does misfire catastrophically is caught at runtime by an automatic
accuracy guard (Section~\ref{sec:eval-guard}).
...
\begin{itemize}
\item \textbf{A JIT optimization runtime} comprising nine rewrite rules, of which
three are validated in isolation (ContextCompress, ModelDowngrade, StateDrop) and two
in composition experiments, with the remaining four implemented but not benchmarked
end to end; we claim empirical savings only for the three rules validated in isolation.
It features a two-stage
...
\item \textbf{An automatic, label-free accuracy guard} that shadow-samples rewritten
calls, measures output divergence against the unrewritten call, and auto-disables a
rule once it drifts beyond budget. It converts a catastrophic StateDrop misfire
($-$42.7\,pp on an unseen agent) into a non-significant delta ($-$0.7\,pp) with no
ground-truth labels, while \textit{preserving} beneficial compression (ContextCompress
stays enabled at 32.4--37.6\% savings on two agents under the guard). With a
normalization-robust divergence metric the guard is \textit{selective}, keeping benign
output-changing rules and still blocking catastrophic ones using a dependency-free signal;
the remaining limit is lexically-invisible semantic disagreement. All guard-on cells use
full shadow sampling, so they establish selectivity under dense feedback rather than
detection delay, damage, latency, or cost at the runtime's 2\% deployment default
(Section~\ref{sec:eval-guard}).
\end{itemize}
...
worker threads; it observes the calls those threads emit and evaluates rules on
each one. Its in-memory profiles and guards are concurrency-safe, while each
\texttt{plan\_audit} row is offered to a bounded non-blocking queue. One
background worker owns the SQLite connection and commits batches of at most
128 rows; a full queue drops the newest diagnostic row rather than delaying a
provider request. Queue, persistence-failure, and drop counters are exposed,
and shutdown uses an ordered flush barrier.

This has three consequences. First, shared state is synchronized rather than
absent: profile access remains concurrent, while audit persistence cannot queue
callers behind SQLite (Section~\ref{sec:eval}). Second,
...
but are not re-consumed in later steps, making them safe to drop. Unlike ContextCompress
and ModelDowngrade, which require only \texttt{agentc.init()}, StateDrop depends on
\texttt{agentc.state\_write}/\texttt{state\_read} annotations to populate these fields;
removing this instrumentation requirement (StateReadWindowPropagation) is future work
...
inflating CC savings by conflating learning signals across configurations. Fix:
...
Three distinct HotpotQA-derived fixtures appear in this paper; Table~\ref{tab:fixtures}
disambiguates them, since they differ in size and construction and back different
claims. The 100\% distractor result (Section~\ref{sec:eval-llmlingua}) is on the
constructed \texttt{hotpot\_distractor} fixture, while \texttt{hotpot\_real} is the
abstention control near the activation boundary.

\begin{table}[t]
\caption{The three HotpotQA-derived fixtures are distinct in size, construction, and
the claim each supports. They must not be conflated: the headline distractor result is
on \texttt{hotpot\_distractor}; \texttt{hotpot\_real} is the abstention control.}
\label{tab:fixtures}
\footnotesize
\begin{tabularx}{\columnwidth}{@{} l r l X @{}}
\toprule
Fixture & $n$ & Size & Role \\
\midrule
\texttt{long\_context\_qa}  & 300 & 13--18\,KB & CC isolation matrix (\S\ref{sec:eval-cc}); 20 expanded paragraphs \\
\texttt{hotpot\_distractor} & 100 & $>$8\,KB   & LLMLingua-2 head-to-head (\S\ref{sec:eval-llmlingua}); 2 supporting + 8 distractors \\
\texttt{hotpot\_real}       & 300 & 6.9\,KB    & Abstention boundary (\S\ref{sec:eval-hotpot}); unmodified split \\
\bottomrule
\end{tabularx}
\end{table}
...
Real-agent accuracy deltas are not statistically significant at either the CC-only or
the composed operating point; on autogen the aggressive CC-only point reaches higher
savings, while composition trades savings (StateDrop preempts ContextCompress) without
affecting accuracy (Section~\ref{sec:eval-realagent}).
...
\midrule
autogen (CC-only)   & real agent (Llama\,3.3-70B)  & CC            & ---$^\S$ & 23.5\% & $-$1.3\,pp (ns, $p$=0.29) & 0.29 \\
autogen (composed)  & real agent (Llama\,3.3-70B)  & CC~+~StateDrop & ---$^\S$ & 14.0\% & $-$1.0\,pp (ns, $p$=0.55) & 0.55 \\
research\_planner (CC)  & real agent        & CC            & 41.7\% & 37.7\% & $+$4.0\,pp (ns) & 0.22 \\
...
\footnotesize{$\S$Together AI cost model not in Agentc pricing table; input-token savings shown in Tok column.}
\end{table*}

The \texttt{plan\_audit.overhead\_us} clock used for the historical
1{,}818-decision result stops before the synchronous audit write and the outer
Python--Rust return. Its 76\,\textmu s pass-through and 120\,\textmu s rewrite
medians are therefore internal diagnostics, not complete interception overhead.
We added a paired release-mode Stage~E0 microbenchmark that wraps the full native
call and aligns every sample with its exact audit row. With the original
synchronous writer, full-call p50/p99 across five 2{,}000-call replications per
path was 0.100/0.165\,ms for guarded reference selection, 0.115/0.307\,ms for an
admitted joint rewrite, and 0.075/0.220\,ms for the legacy greedy rewrite. After
moving SQLite off path, the matched values are 0.073/0.162, 0.085/0.098, and
0.051/0.062\,ms. The paired residual combines the FFI boundary, native-state
lookup, audit-row construction and enqueue, clock quantization, and return
conversion; the ordered SQLite flush is outside the request clock. These E0
measurements are diagnostics, not confirmatory latency claims.

We next vary exact serialized call size (4, 8, 16, 32, and 64\,KiB) and
concurrency (1, 8, and 32 callers) for both guarded-reference and admitted joint-
rewrite paths: five randomized 1{,}024-call replications per cell, or 153{,}600
complete calls per implementation. The original synchronous matrix reaches
1.744--2.388\,ms reference and 2.070--2.853\,ms rewrite p50 at 32 callers, with
14.6--46.1\,ms pooled p99 and only 1.35--1.98$\times$ throughput scaling over one
caller. In the clean-commit off-path rerun, C=32 p50 falls to 0.164--0.184\,ms
and 0.186--0.312\,ms---an 89--93\% matched-cell reduction. Absolute C=32
throughput rises 2.6--4.0$\times$ over the synchronous implementation and scales
2.52--2.75$\times$ for reference selection and 2.58--3.54$\times$ for rewriting
relative to one caller. All 170{,}700 setup-plus-measured audit attempts are
accepted and written with zero reported loss.

The remaining tail is not hidden: pooled C=32 p99 is still 8.96--11.08\,ms for
reference selection and 10.41--16.78\,ms for rewriting on the eight-logical-core
Apple M2. The paired internal p99 is 0.418--1.713\,ms and the residual accounts
for 68--73\% of mean complete-call time, implicating oversubscribed scheduling
and boundary work rather than synchronous SQLite. Twenty-three of 76{,}800
admitted calls (0.030\%) safely return the immutable reference after crossing
the 5\,ms planning deadline. Thus the audit serialization bottleneck is removed,
but the predeclared 1.2\,ms p99 gate remains open pending controlled offered-load
and multi-host measurements.

\begin{figure}[t]
\centering
\includegraphics[width=\columnwidth]{figures/fig7_overhead_scaling.pdf}
\caption{Complete-call latency for an admitted joint rewrite before (dashed)
and after (solid) moving audit persistence off path, across exact serialized
call sizes and concurrent callers (five randomized replications, 5{,}120 calls
per point; log scale). C=32 medians fall 89--93\%, while scheduler/FFI p99
remains 10--17\,ms. Stage~E0, one Apple M2 host, no provider calls; both
artifacts retain the guarded-reference control and replication intervals.}
\label{fig:overhead}
\end{figure}

\begin{figure*}[t]
\centering
\includegraphics[width=\textwidth]{figures/fig8_throughput.pdf}
\caption{End-to-end behavior under concurrent load (\texttt{long\_context\_qa},
\texttt{gpt-4o-mini}, $n$=50 per condition, real API calls). \textit{Left}: per-task
latency CDFs. \textit{Center}: throughput (QPS) and token savings versus concurrency.
\textit{Right}: optimized/baseline p50 and p99 latency ratio. Throughput scales with
concurrency for both baseline and optimized (optimized QPS 1.35$\rightarrow$4.55
$\rightarrow$8.92 across 1/8/32$\times$), and optimized p50 latency is at or below
baseline at every level (ratio $\leq$1.0). Token savings persist under concurrency,
but remote provider latency masks the local contention isolated in
Figure~\ref{fig:overhead}; this single-run $n$=50 experiment does not establish that
the optimizer is free of serialization. The 8$\times$ QPS dip is API variance.}
...
ModelDowngrade savings are structural: price per token, not token count (11.4\%
...
\textbf{Powered accuracy check ($n$=500).} To power the central neutrality claim beyond
$n$=300, we re-run ContextCompress against a shared baseline on an $n$=500 fixture (every
HotpotQA-distractor task expanded above the 8\,KB gate; CC fires 489 of 500). At this
sample size ContextCompress does not merely avoid degradation: it marginally improves
accuracy, $+$2.8\,pp (McNemar exact $p$=0.044, 95\% bootstrap CI $[+0.2,+5.2]$, 14 tasks
broken vs.\ 28 fixed) at 38.2\% input-token savings. The improvement is small and only
marginally significant, but it is directionally consistent with the oracle ceiling
(Section~\ref{sec:eval-oracle}): removing distractor context is mildly beneficial, so
ContextCompress trades no accuracy for its savings on this workload.
...
(300\,MB) run on CPU (no CUDA device was available in our setup); GPU inference would
reduce its absolute overhead but remains orders of magnitude above a dependency-free
IDF pass. Agentc uses its 28\,MB IDF proxy with zero auxiliary inference. All
...
\textbf{Setup.} $n$=300 tasks, \texttt{meta-llama/Llama-3.3-70B-Instruct-Turbo} via
Together AI. The agent makes two LLM calls per task (600 total), accumulating tool
outputs and prior LLM outputs across turns; it writes a tool result to state that the
answer step never re-reads, so both ContextCompress and StateDrop activate. The
shared-baseline ablation uses per-configuration isolated optimizer state and warmup
($W$=30). Together AI pricing is not in Agentc's cost model; input-token savings are
the deterministic attribution signal. ModelDowngrade is inapplicable: no downgrade
route is configured for Llama-3.3-70B, so its \texttt{applies()} precondition blocks
the rule on every call (0 fires).

\textbf{A savings frontier, accuracy-neutral throughout.} On a clean shared baseline
(229 of 300 correct, 76.3\%), both operating points leave accuracy statistically
unchanged (Table~\ref{tab:summary}). Run with ContextCompress alone, CC fires on 350 of
600 calls and reaches 23.5\% per-call input-token savings at 75.0\% accuracy, $-$1.3\,pp,
McNemar exact $p$=0.29 (BF=6, FB=2): not a significant change. In the all-on
configuration the CompositionPlanner runs StateDrop first (260 of 600 calls), removing
stale state-tagged messages and shrinking many prompts below ContextCompress's activation
gate, so CC fires on only 184 calls; accuracy is again neutral at 75.3\%, $-$1.0\,pp,
McNemar $p$=0.55 (BF=7, FB=4), at 14.0\% input-token savings. The frontier here is
therefore between two safe points: the aggressive single-rule point is both accuracy-neutral
and higher-saving, while composition trades roughly a third of the peak savings because
StateDrop preempts the calls ContextCompress would have rewritten. On this agent the
accuracy headroom that motivated composition elsewhere is absent; the safety mechanism that
matters when an aggressive rule \textit{does} misfire is the accuracy guard
(Section~\ref{sec:eval-guard}), not rule composition.

\subsection{StateDrop Generalization and the Automatic Accuracy Guard}
\label{sec:eval-guard}

\textbf{The failure mode generalizes.} StateDrop's read-window heuristic assumes that an
agent forwarding state into a prompt also \texttt{state\_read}s it; when that assumption
is violated, StateDrop drops content the model still uses. On \texttt{research\_planner}
the StateDrop-only configuration degrades accuracy by $-$7\,pp (McNemar $p$=0.065,
$n$=100). To test whether this is fixture-specific, we built a second agent in a
different domain: \texttt{analyst\_qa}, a SQuAD reading-comprehension pipeline
(extract\,$\rightarrow$\,classify\,$\rightarrow$\,answer) that forwards the extracted
context into the answer step without re-reading it. StateDrop there degrades accuracy by
$-$42.7\,pp (49.3\% vs.\ 92.7\% baseline, McNemar $p<10^{-10}$, 95\% CI $[-50.7,-34.7]$,
$n$=150), while the StateDrop-off control is unchanged ($-$0.7\,pp, $p$=1.0). The
degradation is severe precisely because SQuAD answers are fully context-dependent. The
failure mode is a reproducible property of the read-window heuristic, not a quirk of one
fixture.

\textbf{An automatic, label-free accuracy guard.} Agentc shadow-samples a fraction of
rewritten calls: it runs the unrewritten call alongside the optimized one, measures
output divergence (1\,$-$\,Jaccard on output tokens) between them, and folds the
divergence into a per-(call-site, rule) budget. When a rule's divergence exceeds its
budget for five consecutive samples, the planner auto-disables it for that call site.
The mechanism is rule-agnostic and requires no ground-truth labels. Every guard-on
experiment reported below sets the shadow rate to 1.0 so enough comparisons arrive to
characterize a disable decision within $n\leq300$ calls. The runtime's 0.02 default is
a deployment setting, not an operating point evaluated in this paper: these results do
not establish detection delay, damage before disable, or net savings at 2\%. Each sampled
shadow is a second, real, billed API call issued synchronously, and the guard CSVs do not
include that call in their token or cost totals; their savings columns are therefore gross
main-path quantities. The local feedback path is separated from provider overhead in
\path{bench/guard_overhead_bench.py}, but its current result is a single-machine Stage~E0
diagnostic rather than paper evidence, so we do not use it for a quantitative claim.

\textbf{The guard prevents catastrophic misfires.} Re-running \texttt{analyst\_qa} with
the guard active, StateDrop is auto-disabled after roughly ten fires and accuracy
recovers from 49.3\% to 91.3\% ($-$1.3\,pp, McNemar $p$=0.50): the guard converts a
$-$42.7\,pp catastrophe into a non-significant delta, capping damage from 65 broken
tasks to 2 (Table~\ref{tab:guard}).

\textbf{The guard is divergence-selective, and preserves stable savings.} Because the
guard keys on output divergence, it keeps a rule whose optimized output agrees with the
unrewritten output and disables one whose output drifts. ContextCompress on
\texttt{long\_context\_qa} is the agreement case: it removes distractor paragraphs while
preserving the answer-bearing supporting context, so the compressed and original answers
match. With the guard active, ContextCompress is never disabled (fires 289 of 300 calls),
preserving 37.6\% input-token savings at no accuracy cost ($+$0.7\,pp, McNemar $p$=0.83,
95\% CI $[-2.3,+3.7]$, $n$=300). The guard therefore does not simply switch the optimizer
off; it leaves benign, answer-preserving compression intact (Table~\ref{tab:guard}).

\textbf{The divergence metric governs selectivity.} The guard's behavior on benign
output-\textit{changing} rules depends entirely on how divergence is measured, and we trace
this across the full divergence-budget frontier (Fig.~\ref{fig:metric-tradeoff}, $n$=200 per
agent). A raw lexical signal (1\,$-$\,Jaccard on output tokens) detects \textit{change}, not
\textit{degradation}: on \texttt{research\_planner}, where ContextCompress is beneficial but
rewrites the answer text, the lexical guard disables the rule at \textit{every} budget
tested ($\tau\in\{0.10,0.20,0.30,0.50\}$), retaining only 5--63\% of its fires and
collapsing input-token savings to 8.1\% ($\tau$=0.20). Replacing the signal with a
normalization-robust metric, divergence as token \textit{containment} over article- and
punctuation-normalized output, removes this conservatism without any auxiliary model: a
benign elaboration (``Kansas Song'' vs.\ ``Kansas Song (We're From Kansas)'') reads as
agreement while a wrong answer still diverges. Under the normalized metric the guard
\textit{keeps} ContextCompress at every budget (96--100\% of fires retained, 32.4\%
input-token savings preserved, accuracy neutral at $\pm$0.0\,pp, $\tau$=0.20) and
\textit{still} auto-disables catastrophic StateDrop on \texttt{analyst\_qa} (at $n$=200 the
unguarded misfire deepens to $-$49.5\,pp and the guard caps it to $-$3.5\,pp, $\sim$91\% of
the damage prevented). \textbf{No lexical operating point reaches the benign-and-safe region
across this 10$\times$ range of budgets, while the normalized metric occupies it at every
budget} (Fig.~\ref{fig:metric-tradeoff}): the naive metric cannot preserve a benign
output-changing rule, whereas the selective metric can. The remaining ceiling is genuinely
semantic disagreement that is lexically invisible; a learned divergence meter would tighten
it further, at the cost of the dependency-free property.

\textbf{The selectivity generalizes across model families, at a per-model budget.} We
replicate the benign/catastrophic selectivity on three further families spanning different
vendors and architectures, each at $n$=100: Llama-3.3-70B (Meta), Claude Haiku~4.5
(Anthropic), and Qwen3-235B (Alibaba), via OpenAI-compatible endpoints
(Table~\ref{tab:xmodel}). Across all four families the normalized metric makes the correct
keep/disable decision: it preserves benign ContextCompress at its full firing rate (93 of 100
calls, $\sim$30--32\% input-token savings) while the lexical metric over-disables it. The
catastrophic StateDrop misfire is disabled on every model; recovery completeness varies with
the model (35--93\% of the damage prevented) because it depends on how quickly the
five-sample disable triggers, which is a function of the model's output distribution. That
same dependence makes the divergence budget a \textbf{per-model hyperparameter}: the three
instruct models operate at $\tau$=0.20, while Qwen3-235B's more verbose output regime
calibrates to a tighter $\tau$=0.10, where it reproduces the full pattern (lexical collapses
benign savings to 1.7\%, normalized catches StateDrop at 66\% of the damage, up from 20\% at
$\tau$=0.20) --- confirming the apparent anomaly was an operating-point shift, not a failure
of the mechanism. The selectivity is architecture-general; only the operating budget is
calibrated per model.

\begin{table}[t]
\caption{The shadow-divergence accuracy guard is \textbf{selective}, and its selectivity
is set by the divergence metric. It keeps stable benign compression (CC on
\texttt{long\_context\_qa}, $n$=300) and catches catastrophic misfires (StateDrop on
\texttt{analyst\_qa}, $n$=150) under either metric. For benign \textit{output-changing}
compression (CC on \texttt{research\_planner}, $n$=200, operating point $\tau$=0.20) the
lexical metric is over-conservative (disables the rule, savings collapse to 8.1\%) while the
normalized containment metric preserves it (32.4\% retained) and still catches StateDrop;
this holds across the full divergence-budget frontier (Fig.~\ref{fig:metric-tradeoff}). The
$n$=200 \texttt{research\_planner} guard rows are the frontier-sweep configuration; the
$n$=100 per-agent evaluation finds CC mildly beneficial ($+$4\,pp), consistent with benign
compression. All guard-on accuracy deltas are non-degrading. Every guard-on row uses
full shadow sampling (rate 1.0); the table does not measure
2\% deployment behavior or net shadow cost.}
\label{tab:guard}
\small
\begin{tabular}{lllr}
\toprule
Agent / rule & Guard & Acc $\Delta$ & Token savings \\
\midrule
long\_context\_qa / CC  & on            & $+$0.7\,pp (ns) & 37.6\% (kept) \\
analyst\_qa / StateDrop & off           & $-$42.7\,pp     & --- \\
analyst\_qa / StateDrop & on (lexical)  & $-$1.3\,pp (ns) & disabled \\
analyst\_qa / StateDrop & on (norm.)    & $-$0.7\,pp (ns) & disabled \\
research\_planner / CC  & off           & $-$1.5\,pp (ns) & 33.8\% \\
research\_planner / CC  & on (lexical)  & $-$1.0\,pp (ns) & 8.1\% (disabled) \\
research\_planner / CC  & \textbf{on (norm.)} & \textbf{$\pm$0.0\,pp (ns)} & \textbf{32.4\% (kept)} \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{figure}[t]
\centering
\includegraphics[width=\columnwidth]{figures/fig9_metric_tradeoff.pdf}
\caption{The divergence metric decides the benign/safety tradeoff. Each point is one
(metric, budget~$\tau$) configuration ($n$=200 per agent): $x$ is the benign rule retained
(guarded CC fires\,/\,unguarded), $y$ is the catastrophic damage prevented (one minus the
ratio of guarded to unguarded StateDrop accuracy loss). The lexical metric catches the
catastrophe at every budget ($y\!\approx\!92\%$) but disables the benign rule at every
budget ($x\!=\!5$--$63\%$, top-left); the normalized containment metric occupies the
benign-and-safe region at every budget ($x\!\ge\!96\%$, top-right). Unguarded keeps the
benign rule but prevents no damage (bottom-right). No lexical operating point reaches the
desired region. All guard-on points use full shadow sampling; the figure characterizes
metric selectivity under dense feedback, not detection delay at the 2\% deployment
default.}
\label{fig:metric-tradeoff}
\end{figure}

\begin{table}[t]
\caption{The metric selectivity generalizes across \textbf{four model families} at a
per-model divergence budget $\tau$. Normalized makes the correct keep/disable decision on all
four---benign ContextCompress (on \texttt{research\_planner}) kept at its full firing rate,
catastrophic StateDrop (on \texttt{analyst\_qa}) disabled---while lexical over-disables benign
CC everywhere. Catastrophe-recovery completeness is model-dependent (35--93\% of the unguarded
damage prevented). Qwen3-235B's more verbose output regime calibrates to a tighter
$\tau$=0.10. Every guard-on cell uses full shadow sampling. $n$=100 ($n$=200 for
gpt-4o-mini).}
\label{tab:xmodel}
\small
\begin{tabular}{llllr}
\toprule
Model (vendor) & $\tau$ & Lexical (benign CC) & Normalized (benign CC) & SD prev. \\
\midrule
gpt-4o-mini (OpenAI)       & 0.20 & disabled, 8\%   & \textbf{kept, 32\%} & 93\% \\
Llama-3.3-70B (Meta)       & 0.20 & disabled, 1.6\% & \textbf{kept, 30\%} & 35\% \\
Claude Haiku 4.5 (Anthropic) & 0.20 & disabled, 17\% & \textbf{kept, 31\%} & 73\% \\
Qwen3-235B (Alibaba)       & 0.10 & disabled, 1.7\% & \textbf{kept, 31\%} & 66\% \\
\bottomrule
\end{tabular}
\end{table}
...
\textbf{Real-agent operating points.} autogen\_bridge is evaluated at $n$=300 on
Llama-3.3-70B under the warmup-corrected harness with a clean shared baseline. Both the
CC-only ($-$1.3\,pp, $p$=0.29, 23.5\% savings) and the composed ($-$1.0\,pp, $p$=0.55,
14.0\% savings) configurations are accuracy-neutral; the two points form a savings
frontier, not a savings/accuracy tradeoff, with composition trading peak savings because
StateDrop preempts ContextCompress (Section~\ref{sec:eval-realagent}).
debug\_agent is evaluated without accuracy tracking (no per-task scoring fixture); its
results are cost and token savings only. research\_planner is evaluated with EM accuracy
($n$=100): ContextCompress is beneficial there ($+$4\,pp, 37.7\% savings) while StateDrop
degrades accuracy ($-$7\,pp) by dropping forwarded-but-unread state, the failure mode the
accuracy guard targets (Section~\ref{sec:eval-guard}).

\textbf{The accuracy guard's selectivity is bounded by its divergence metric.} The
shadow-divergence guard keys on change from the unrewritten output. A normalization-robust
containment metric makes it selective, preserving benign output-changing compression
(ContextCompress on \texttt{research\_planner}, 32.4\% savings retained at $n$=200) while
still disabling catastrophic StateDrop, with no auxiliary model. Its remaining ceiling is
genuine semantic disagreement that is lexically invisible (two correct answers with no
token overlap would read as divergence); a learned divergence meter would close this at the
cost of the dependency-free property. The threshold and metric tune the safety/savings
tradeoff, and label-free gating still cannot certify accuracy without ground truth.

\textbf{Guard sampling and controller scope.} Every guard-on result in this paper
uses a shadow rate of 1.0 to obtain dense feedback within a short experimental run.
The results demonstrate metric selectivity conditional on that feedback; they do not
measure calls or time to disable, cumulative task damage, false disables, request-tail
latency, or net cost at the runtime's 0.02 default. Shadow calls are absent from the
reported token and cost totals. Moreover, these CSVs evaluate the predecessor per-rule,
five-breach controller; the artifact's newer complete-plan exposure controller requires
a fresh rate-sweep evaluation before it can support a paper claim.
...
shared-baseline ablation methodology are each observable in this evaluation: CC
...
change at either its aggressive single-rule or its composed operating point; the rare
rule that misfires catastrophically is caught at runtime by an automatic, label-free
accuracy guard (Section~\ref{sec:eval-guard}). Input-token savings are within 1\,pp across
...
pairs). A paired 307{,}200-call Stage~E0 scaling before/after diagnostic shows that moving
audit persistence behind a bounded non-blocking queue cuts matched C=32 medians
89--93\% and raises absolute throughput 2.6--4.0$\times$ with zero reported audit
loss. C=32 p99 remains 9.0--16.8\,ms on the oversubscribed eight-logical-core host,
so the 1.2\,ms ship gate and confirmatory multi-host/provider-level latency
measurements remain open. The methodology introduced here, comprising
shared-baseline ablation,
...
\texttt{llmlingua\_\allowbreak accuracy\_\allowbreak n100.csv},
\texttt{agentc\_\allowbreak hotpot\_\allowbreak n100.csv} (Agentc-CC row),
\texttt{wikipedia\_\allowbreak qa\_\allowbreak comparison.csv} &
\texttt{python -m bench.agents.llmlingua\_\allowbreak baseline} \\
...
\texttt{generalization\_\allowbreak evals.csv} (support\_qa row),
\texttt{generalization\_\allowbreak activation.csv} &
\texttt{python -m bench.run\_\allowbreak generalization\_\allowbreak evals} \\

\addlinespace
\ref{sec:eval-cachehit} &
\texttt{traces.db} (\texttt{plan\_audit}, \texttt{plan\_kind='cached'}) &
Run \texttt{python -m bench.agents.batch\_\allowbreak classifier} twice; the
in-script \texttt{agentc.shutdown()} seeds the cache between runs \\

\addlinespace
\ref{tab:summary} (research\_planner) &
\texttt{research\_\allowbreak planner\_\allowbreak warmup\_\allowbreak n150.csv} &
\texttt{python -m bench.run\_\allowbreak research\_\allowbreak planner\_\allowbreak warmup} \\

\addlinespace
\ref{tab:summary} (autogen rows) &
\texttt{autogen\_\allowbreak bridge\_\allowbreak warmup\_\allowbreak n300.csv} &
\texttt{AB\_\allowbreak CONFIGS=...\ python -m bench.run\_\allowbreak autogen\_\allowbreak warmup\_\allowbreak n300} \\

\addlinespace
\ref{tab:guard} (SD generalization) &
\texttt{analyst\_\allowbreak qa\_\allowbreak sd\_\allowbreak gen.csv} &
\texttt{python -m bench.build\_\allowbreak squad\_\allowbreak fixture}; \texttt{python -m bench.run\_\allowbreak analyst\_\allowbreak sd\_\allowbreak generalization} \\

\addlinespace
\ref{tab:guard} (guard on/off), \ref{fig:metric-tradeoff} &
\texttt{gsweep\_\allowbreak tradeoff\_\allowbreak *.csv} &
\texttt{OPENAI\_\allowbreak API\_\allowbreak KEY=...\ bash bench/repro/guard\_\allowbreak frontier.sh};
\texttt{python bench/paper\_\allowbreak figures/fig9\_\allowbreak metric\_\allowbreak tradeoff.py} \\

\addlinespace
\ref{tab:xmodel} &
\texttt{gsweep\_\allowbreak \{xmodel,claude,qwen3\}\_\allowbreak *.csv} &
\texttt{bash bench/repro/crossmodel\_\allowbreak selectivity.sh} (one invocation per model family; see \texttt{bench/repro/README.md}) \\

\addlinespace
\S\ref{sec:eval-guard} (guard overhead) &
Stage~E0 local diagnostic (no API; not paper evidence) &
\texttt{python -m bench.guard\_\allowbreak overhead\_\allowbreak bench} \\

\addlinespace
Optimizer complete-call overhead &
\texttt{optimizer-e2e-overhead[-offpath-audit]-2026-09-04.\{json,csv\}} (Stage~E0) &
\texttt{python -m bench.optimizer\_\allowbreak e2e\_\allowbreak overhead} \\

\addlinespace
Optimizer size/concurrency scaling &
\texttt{optimizer-e2e-scaling[-offpath-audit]-2026-09-04.\{json,csv.gz\}} (Stage~E0) &
\texttt{python -m bench.optimizer\_\allowbreak e2e\_\allowbreak scaling} \\

\addlinespace
\ref{fig:throughput} &
\texttt{concurrency\_\allowbreak bench\_\allowbreak summary.csv} &
\texttt{python -m bench.run\_\allowbreak concurrency\_\allowbreak bench --concurrency 1 8 32};
\texttt{python bench/paper\_\allowbreak figures/fig\_\allowbreak concurrency.py} \\
