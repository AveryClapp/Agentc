...
ground-truth labels. We also characterize its limit: because divergence measures change
rather than degradation, it conservatively suppresses benign output-altering savings
(a tunable safety/savings tradeoff), which is the fundamental ceiling of label-free
runtime gating (Section~\ref{sec:eval-guard}).
\end{itemize}
...
\textbf{The guard is conservative, by construction.} Because divergence is measured
against the unrewritten output, the guard detects \textit{change}, not
\textit{degradation}: it cannot distinguish a harmful drop from a beneficial one. On
\texttt{research\_planner}, where ContextCompress is beneficial (removing distractors,
$+$4\,pp, 37.7\% token savings), the guard disables it after 37 fires, cutting savings
to 11.0\% ($-$1.0\,pp). The guard therefore provides a safety floor at a savings cost,
governed by the divergence threshold. This is the fundamental limit of label-free
runtime accuracy gating: it can bound degradation but cannot preserve every benign
optimization.

\begin{table}[t]
\caption{The shadow-divergence accuracy guard auto-disables a rule after five
consecutive over-budget divergence samples. It prevents catastrophic degradation
(StateDrop on \texttt{analyst\_qa}) but conservatively suppresses benign
output-altering savings (ContextCompress on \texttt{research\_planner}); the threshold
tunes the safety/savings tradeoff.}
\label{tab:guard}
\small
\begin{tabular}{llrr}
\toprule
...
analyst\_qa / StateDrop      & \textbf{on}  & \textbf{$-$1.3\,pp (ns)} & --- \\
\midrule
research\_planner / CC       & off          & $+$4.0\,pp (ns)      & 37.7\% \\
research\_planner / CC       & \textbf{on}  & $-$1.0\,pp (ns)      & \textbf{11.0\%} \\
\bottomrule
\end{tabular}
\end{table}
...
\textbf{The accuracy guard is conservative.} The shadow-divergence guard measures change
from the unrewritten output, so it bounds degradation but cannot distinguish beneficial
from harmful change. At strict thresholds it disables benign output-altering rules
(ContextCompress on \texttt{research\_planner}, 37.7\%\,$\rightarrow$\,11.0\%). The
divergence threshold tunes the safety/savings tradeoff; label-free runtime gating cannot
preserve every benign optimization.
