...
%%  [T4] throughput-under-concurrency figure -> fig:throughput (commented, near fig:overhead)
%%       src: bench/paper_results/concurrency_bench_summary.csv
...
%% DROPIN [T4]: end-to-end throughput/latency under concurrency. Uncomment once
%% figures/fig8_throughput.pdf exists (generated post-experiment from
%% bench/paper_results/concurrency_bench_summary.csv via run_concurrency_bench).
%% This is the systems-venue metric R3 asked for (QPS scaling + p99, optimized vs baseline).
%% \begin{figure}[t]
%% \centering
%% \includegraphics[width=\columnwidth]{figures/fig8_throughput.pdf}
%% \caption{End-to-end throughput scales with concurrency for both baseline and
%% optimized runs, confirming the optimizer is not a serialization point
%% (Section~\ref{sec:decoupling}). QPS at 1/8/32$\times$ concurrency with p99 latency overlay.}
