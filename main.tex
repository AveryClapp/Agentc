...
output-\textit{changing} rules depends entirely on how divergence is measured. A raw
lexical signal (1\,$-$\,Jaccard on output tokens) detects \textit{change}, not
\textit{degradation}: on \texttt{research\_planner}, where ContextCompress is beneficial
but rewrites the answer text, the lexical guard disables it after 37 fires, cutting savings
from 37.7\% to 11.0\%. Replacing the signal with a normalization-robust metric, divergence
as token \textit{containment} over article- and punctuation-normalized output, removes this
conservatism without any auxiliary model: a benign elaboration (``Kansas Song'' vs.\
``Kansas Song (We're From Kansas)'') reads as agreement while a wrong answer still diverges
(0.7--1.0). Under the normalized metric the guard \textit{keeps} ContextCompress on
\texttt{research\_planner} (94 fires, 37.5\% savings preserved, accuracy non-degrading) and
\textit{still} auto-disables catastrophic StateDrop on \texttt{analyst\_qa} (recovering to
$-$0.7\,pp). The guard is therefore \textbf{selective}: it preserves beneficial compression
and blocks catastrophic misfires using a dependency-free signal (Table~\ref{tab:guard}).
The remaining ceiling is genuinely semantic disagreement that is lexically invisible; a
learned divergence meter would tighten it further, at the cost of the dependency-free
property.
...
\texttt{long\_context\_qa}) and catches catastrophic misfires (StateDrop on
\texttt{analyst\_qa}) under either metric. For benign \textit{output-changing} compression
(CC on \texttt{research\_planner}) the lexical metric is over-conservative (disables,
11.0\%) while the normalized containment metric preserves it (37.5\%) and still catches
StateDrop. All guard-on accuracy deltas are non-degrading.}
\label{tab:guard}
\small
\begin{tabular}{lllr}
\toprule
...
research\_planner / CC  & off           & $+$4.0\,pp (ns) & 37.7\% \\
research\_planner / CC  & on (lexical)  & $-$1.0\,pp (ns) & 11.0\% (disabled) \\
research\_planner / CC  & \textbf{on (norm.)} & \textbf{$+$9.0\,pp} & \textbf{37.5\% (kept)} \\
\bottomrule
\end{tabular}
\end{table}
...
(ContextCompress on \texttt{research\_planner}, 37.5\% savings retained) while still
disabling catastrophic StateDrop, with no auxiliary model. Its remaining ceiling is
