...
Separately, the historical internal-clock context-size sweep grows from
0.33\,ms to 0.71\,ms at p50 over a 16$\times$ size range (4\,KB to 64\,KB); it
does not measure the full FFI path. The flat overhead profile under concurrency
is attributable to the execution-optimizer decoupling
...
a serialization point under parallel workloads. Figure~\ref{fig:throughput} confirms
this end to end: across 1/8/32$\times$ concurrency, optimized throughput scales with
baseline and optimized p50 latency stays at or below baseline.

\begin{figure}[t]
\centering
\includegraphics[width=\columnwidth]{figures/fig7_overhead_scaling.pdf}
\caption{Historical internal planner-clock diagnostic versus context length.
p50 grows from 0.33\,ms to 0.71\,ms over a 16$\times$ size range
(4\,KB to 64\,KB); p95 stays under 1.5\,ms throughout. This legacy
clock excludes the outer FFI boundary and audit persistence. Its isolated
8\,KB p99 tail therefore cannot be attributed to the audit write.}
\label{fig:overhead}
\end{figure}
...
baseline at every level (ratio $\leq$1.0), confirming the optimizer does not become a
serialization point under parallel load (Section~\ref{sec:decoupling}). Token savings
persist under concurrency. The 8$\times$ QPS dip is single-run API variance at $n$=50.}
...
pairs). A paired complete-call Stage~E0 diagnostic stays below 0.4\,ms at p99 on
the recorded host; confirmatory context-size and concurrency measurements remain
part of the frozen evaluation campaign. The methodology introduced here, comprising
