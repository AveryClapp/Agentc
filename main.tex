...
\texttt{plan\_audit} row is synchronously appended through one mutex-protected
SQLite connection.

This has three consequences. First, shared state is synchronized rather than
absent: profile access remains concurrent, but audit persistence can queue callers
under load (Section~\ref{sec:eval}). Second,
...
call and aligns every sample with its exact audit row. Across five 2{,}000-call
replications per path, full-call p50/p99 was 0.100/0.165\,ms for guarded reference
selection, 0.115/0.307\,ms for an admitted joint rewrite, and 0.075/0.220\,ms for
the legacy greedy rewrite. The paired internal medians were 0.062, 0.071, and
0.042\,ms, respectively. Their difference combines the FFI boundary, native-state
lookup, audit serialization and commit, clock quantization, and return conversion;
it is not an audit-only timer. These E0 measurements keep p99 below 1\,ms on the
recorded host, but the frozen protocol excludes them from confirmatory latency
claims.
...
complete calls. At one caller, p50 spans 0.108--0.135\,ms for reference selection
and 0.128--0.253\,ms for an admitted rewrite. At 32 callers, those ranges become
1.744--2.388\,ms and 2.070--2.853\,ms; pooled p99 reaches 14.6--41.4\,ms and
23.7--46.1\,ms, respectively. Throughput improves only 1.35--1.69$\times$ for
reference selection and 1.38--1.98$\times$ for rewriting relative to one caller
on an eight-core Apple M2. The paired internal planner p99 remains at or below
0.721\,ms at 32 callers, while the combined boundary/state/audit residual accounts
for 94--97\% of mean complete-call time. This residual is not an audit-only clock,
but the synchronous mutex-protected audit append is the known serialized operation
in its scope. Two of 76{,}800 admitted-path calls (0.0026\%) safely fell back with
\texttt{planning\_overhead\_exceeded}. These results expose a local persistence
bottleneck; they remain single-machine Stage~E0 diagnostics rather than
confirmatory latency evidence.

\begin{figure}[t]
\centering
\includegraphics[width=\columnwidth]{figures/fig7_overhead_scaling.pdf}
\caption{Complete-call latency for an admitted joint rewrite across exact
serialized call sizes and concurrent callers (five randomized replications,
5{,}120 calls per point; log scale). Sequential medians remain below 0.3\,ms,
but the synchronous production path develops millisecond medians and
10--50\,ms-scale p99 tails under contention. Stage~E0, one Apple M2 host, no
provider calls; replication-level intervals and the guarded-reference control
...
pairs). A paired fixed-shape Stage~E0 diagnostic stays below 0.4\,ms at p99, while
a 153{,}600-call size/concurrency diagnostic exposes the synchronous audit append as
a scalability target: 32-caller p50 is 1.7--2.9\,ms and p99 is 14.6--46.1\,ms on
the recorded host. Confirmatory multi-host and provider-level latency measurements
remain part of the frozen evaluation campaign. The methodology introduced here, comprising
...
\texttt{optimizer-e2e-overhead-2026-09-04.\{json,csv\}} (Stage~E0) &
\texttt{python -m bench.optimizer\_\allowbreak e2e\_\allowbreak overhead} \\

\addlinespace
Optimizer size/concurrency scaling &
\texttt{optimizer-e2e-scaling-2026-09-04.json} and \texttt{.csv.gz} (Stage~E0) &
