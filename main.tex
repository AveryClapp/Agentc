...
auto-disables thereafter incurs no further shadow cost. The local feedback path is
separated from provider overhead in \path{bench/guard_overhead_bench.py}, but its current
result is a single-machine Stage~E0 diagnostic rather than paper evidence, so we do not
use it for a quantitative claim. The
