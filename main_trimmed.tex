...
% TRIM-CONDENSE: tighten ~30-50% for conference length
...
\section{Evaluation}
%% TRIM: per-rule validations + CacheHit/proxy/cold-agent moved to Appendix~\ref{app:eval}
\label{sec:eval}
...
auto-disables thereafter incurs no further shadow cost. The per-sample bookkeeping is
...
\path{bench/guard_overhead_bench.py}), five orders of magnitude below an LLM call, so the
guard adds no measurable latency to the request path.
...
% ===== Relocated for conference length (see main.tex for full version) =====
