...
| `LIT-008` | Optimizing Model Selection for Compound AI Systems | per-component model choice | compound-system model selector | explicit model-selection layer | offline/online mixed | compound systems | cost/quality | quality | direct model-selection comparison; AgentC spans more rewrite classes |
...
| `LIT-023` | ALTO | distributed compound pipeline execution | orchestrator/scheduler | no public artifact verified | runtime | compound pipelines | latency/throughput | pipeline output quality | systems comparison for orchestration, not semantic rewrite suite |
