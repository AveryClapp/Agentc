...
| `WP-005` | `literature` | Nearest-neighbor comparison needs final cleanup. | Verified blurbs now identify the main threats. | Promote metadata and tighten comparison language around Agentix/Halo/Murakkab/AIOS/Cognify/SGLang/vCache. | `literature-verified-blurbs.md`, `nearest-neighbor-comparison.md` | `in-progress` |
...
| `WP-009` | `literature` | Novelty claim is too broad unless narrowed against closest systems. | Mark "first runtime optimizer" as unsafe. | Use verified nearest-neighbor blurbs to write the exact distinction: framework-call interception plus multi-rule trace rewrites. | `nearest-neighbor-comparison.md`, `claim-bank.md`, `current-fit-and-publishability.md` | `open` |
| `WP-010` | `literature` | StateDrop has analogy support but not a full soundness story. | Treat StateDrop as promising and cite program-analysis/liveness work carefully. | Define AgentC's dependency/read-window model before making principled StateDrop claims. | `citation-gap-list.md`, `related-work-map.md`, section brief | `open` |
