...
| `ART-056` | `paper-intelligence/literature-blurb-todo.md` | `md` | created locally from current `LIT` candidates | pending | `tracked-candidate` | high | `LIT-002`, `LIT-040`, `GAP-009`, `GAP-010`, `GAP-012`, `GAP-013`, `GAP-014`, `GAP-016` | Normalized source-blurb pass for `LIT-002` through `LIT-070`, with evidence scores, novelty-threat scores, baseline decisions, and verification queue. |
