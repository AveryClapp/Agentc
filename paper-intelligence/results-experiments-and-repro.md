...
| `RES-013` | diagnostic | Optimizer complete-call overhead | three synthetic steady-state paths | 10,000/path | n/a | `bench/repro/optimizer-e2e-overhead-2026-09-04.{json,csv}`, `bench/paper_results/optimizer_overhead.txt` | complete-call p50/p99: guarded reference 100/165µs; admitted joint rewrite 115/307µs; legacy greedy rewrite 75/220µs. Paired internal medians: 62/71/42µs. | Scope correction and Stage E0 systems diagnostic only | Single arm64 host, no provider/network/concurrency; `paper_evidence=false`. The older 76/120µs values and context-size curve use a pre-audit internal clock and cannot support an end-to-end claim. |
...
- Use `RES-013` only as a Stage E0 timing-scope diagnostic. Do not promote its numbers to a confirmatory latency claim; rerun complete-call context-size and concurrency cells under the frozen campaign.
...
| `EXP-007` | diagnostic corrected | `GAP-015` | Optimizer overhead measurement. | `RES-013`: 30,000 paired complete-call samples; p99 below 0.4ms for all three measured steady-state paths. Historical 76/120µs values are now labeled pre-audit internal timings. | Run the complete-call context-size and concurrency matrix (`bd-tp3f`) before any confirmatory paper claim. |
