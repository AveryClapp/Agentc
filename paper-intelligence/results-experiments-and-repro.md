...
| `RES-013` | diagnostic | Optimizer complete-call overhead | three synthetic steady-state paths | 10,000/path | n/a | `bench/repro/optimizer-e2e-overhead-2026-09-04.{json,csv}`, `bench/paper_results/optimizer_overhead.txt` | complete-call p50/p99: guarded reference 104/351µs; admitted joint rewrite 118/385µs; legacy greedy rewrite 76/287µs. Paired internal medians: 65/73/44µs. | Scope correction and Stage E0 systems diagnostic only | Single arm64 host, no provider/network/concurrency; `paper_evidence=false`. The older 76/120µs values and context-size curve use a pre-audit internal clock and cannot support an end-to-end claim. |
