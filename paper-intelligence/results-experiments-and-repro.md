...
| `RES-013` | diagnostic | Optimizer complete-call overhead and scaling | three fixed-shape paths plus exact 4/8/16/32/64 KiB calls at C=1/8/32 | 30,000 fixed-shape + 153,600 scaling calls | n/a | `bench/repro/optimizer-e2e-overhead-2026-09-04.{json,csv}`, `bench/repro/optimizer-e2e-scaling-2026-09-04.{json,csv.gz}`, `bench/paper_results/optimizer_overhead.txt`, `bench/paper_results/optimizer_e2e_scaling.txt` | Fixed-shape complete-call p50/p99: guarded reference 100/165µs; admitted joint rewrite 115/307µs; legacy greedy rewrite 75/220µs. In the scaling matrix, C=1 p50 is 108–135µs reference and 128–253µs rewrite; C=32 p50/p99 is 1.74–2.39ms/14.6–41.4ms reference and 2.07–2.85ms/23.7–46.1ms rewrite. Throughput speedup is only 1.35–1.98×. Internal C=32 p99 is at most 721µs; residual is 94–97% of mean. Two of 76,800 admitted calls safely hit the 5ms planning fallback. | Scope correction, honest negative systems result, and audit-persistence optimization target | One Apple M2 host, synthetic calls, no provider/network; `paper_evidence=false`. Tail intervals are wide across five scaling replications. The older 76/120µs values and context-size curve use a pre-audit internal clock and cannot support an end-to-end claim. |
...
- Use both artifacts under `RES-013` as Stage E0 timing diagnostics. The fixed-shape run isolates path cost; the size/concurrency run shows that synchronous audit persistence destroys sub-millisecond tails under contention. Do not promote either to a confirmatory latency claim.
...
| `EXP-007` | diagnostic corrected | `GAP-015` | Optimizer overhead measurement. | `RES-013`: 30,000 fixed-shape paired calls plus 153,600 size/concurrency calls; sequential medians stay below 0.3ms, but C=32 p99 reaches 46.1ms and throughput scales at most 1.98×. Historical 76/120µs values remain pre-audit internal timings. | Remove or batch the synchronous audit write, rerun the frozen scaling artifact, then execute the confirmatory campaign. |
