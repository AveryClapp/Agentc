...
| `RR-009` | `high` | ModelDowngrade may look like ordinary model routing/cascading. | AgentC routes internal call sites in traces, not just user queries. | RouteLLM/FrugalGPT/cascade verification and call-site policy evidence. | Present routing as one runtime pass inside a broader optimizer. | `GAP-010`, `GAP-012` |
...
| `RR-014` | `high` | Single-run evaluation is underpowered for stochastic LLM behavior. | Current result ledger is useful but needs paired uncertainty or repeated-run framing. | tau-bench/ReliableEval/HELM/MT-Bench verification plus row-level analysis. | Elevate `GAP-014`; avoid “proves behavior preservation” wording. | `GAP-004`, `GAP-014` |
